...
export default function EcoDrivingPage() {
  ...
  const [scores, setScores] = useState<EcoDrivingScore[]>([]);
  ...
  const refresh = useCallback(async (h: number) => {
    ...
    setError("");
    try {
      const data = await getEcoDrivingScores(h);
      setScores(data);
      setLastUpdated(new Date());
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : "Error cargando datos de conducción");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const periodLabel = PERIODS.find(p => p.hours === hours)?.label ?? `${hours}h`;

  // Average fleet score
  const withData = scores.filter(s => s.total_records > 0);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        <div>
          <h1 className="text-lg font-bold text-white">Conduccion Eficiente</h1>
          <p className="text-xs" style={{ color: "var(--muted)" }}>
            {lastUpdated
              ? `Actualizado: ${lastUpdated.toLocaleTimeString("es-ES")} — Periodo: ultimas ${periodLabel}`
              : "Cargando..."}
            {avgScore != null && ` — Puntuacion media flota: `}
            {avgScore != null && (
        ...
        >
          {PERIODS.map(p => (
            <button
              ...
              style={{
                background: hours === p.hours ? "var(--accent)" : "transparent",
                color: hours === p.hours ? "#fff" : "var(--muted)",
              }}
            >
              {p.label}
            </button>
      ...
      <div className="flex-1 overflow-auto px-6 py-4 space-y-6">
        ...
        {/* Ranking table */}
        {!loading && <RankingTable scores={scores} />}
      </div>
    </div>
  );
}
